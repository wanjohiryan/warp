...
import { LogoName, Logo, Gpad as Controller } from "../svg";
import { Toast } from "../util";

type Props = {
    onPressFeedback: () => void;
    // onSetBitrate: (v: number) => void;
    latency:number
}

const MAX_BITRATE = 3

export default function Sidebar({ onPressFeedback, latency }: Props) {
    ...
    const handleButtonClick = (index: number, clicked = true) => {
        buttonsRef.current.forEach((button: { querySelector: (arg0: string) => any; }, i: any) => {
        ...
        if (clicked) {
            ...
            // onSetBitrate((sliderValue * MAX_BITRATE) / 100)
        }
    };
