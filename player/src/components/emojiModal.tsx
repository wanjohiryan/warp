...
export default function EmojiModal() {
    ...
    const handleButtonClickStream = (index: string | number) => {
        buttonsStreamRef.current.forEach((button: { querySelector: (arg0: string) => any; }, i: string | number) => {
                        ...
                        <div className="col-4">
                            {emojiNames.map((name, index) => (
                                <button key={`emoji-${index}`} className="emoji_button">
                                    <img loading="eager" crossOrigin="anonymous" src={`https://raw.githubusercontent.com/MKAbuMattar/fluentui-emoji/main/icons/modern/${name}.svg`} />
                                </button>
                            ))}
                        </div>
                    </div>
