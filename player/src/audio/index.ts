...
export default class Audio {
    ...
    constructor() {
        ...
        const config = {
            sampleRate: 44100,
            ring: new RingInit(2, 4410 * 5), // 500ms at 44.1khz
        }
    ...
    private setupWorker(config: Message.Config): Worker {
        const url = new URL('audioWorker.ts', import.meta.url)
    ...
    private async setupWorklet(config: Message.Config): Promise<AudioWorkletNode> {
        ...
        const url = new URL('audioWorklet.ts', import.meta.url)
